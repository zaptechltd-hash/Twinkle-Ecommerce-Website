...
function AuthModal({ onClose, onLogin, onLogout, user }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-white w-full max-w-xs mr-6 md:mr-12 shadow-sm" onClick={(e) => e.stopPropagation()}>
        {user ? (
          <div className="p-8">
            <div className="flex items-center gap-4 mb-6 pb-5 border-b border-stone-100">
              ...
              <div className="overflow-hidden">
                <p className="text-[12px] font-medium text-stone-800 truncate">{user.name}</p>
                <p className="text-[11px] text-stone-400 truncate">{user.email}</p>
              </div>
            </div>
            {["My Orders", "My Wishlist", "Account Settings"].map((item) => (
              <button key={item} className="block w-full text-left py-3 text-[11px] tracking-[0.15em] uppercase text-stone-600 border-b border-stone-100 hover:text-stone-900 transition-colors">
                {item}
              </button>
            ))}
            <button onClick={() => { onLogout(); onClose(); }} className="block w-full text-left py-3 text-[11px] tracking-[0.15em] uppercase text-red-400 hover:text-red-600 transition-colors mt-1">
              Sign Out
            </button>
          </div>
        ) : (
          <div className="p-8">
            <div className="flex gap-0 mb-7 border-b border-stone-100">
              {[{ key: "login", label: "Sign In" }, { key: "signup", label: "Register" }].map((m) => (
            ...
            {mode === "signup" && (
              <>
                <label className="block text-[10px] tracking-[0.2em] uppercase text-stone-400 mb-1.5">Full Name</label>
                <input type="text" placeholder="Sara Noor" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
            ...
            <label className="block text-[10px] tracking-[0.2em] uppercase text-stone-400 mb-1.5">Email</label>
            ...
            {error && <p className="text-[10px] text-red-400 tracking-wide mb-3">{error}</p>}
            <button onClick={handleSubmit} className="w-full py-3.5 bg-stone-900 text-white text-[11px] tracking-[0.3em] uppercase hover:bg-stone-700 transition-colors">
              {mode === "login" ? "Sign In" : "Create Account"}
            </button>
            {mode === "login" && (
              <p className="text-[10px] text-stone-400 text-center mt-4 tracking-wide">
                Forgot password?{" "}
                <span className="underline cursor-pointer hover:text-stone-700 transition-colors">Reset</span>
              </p>
            )}
          </div>
        )}
    ...
    <div className="fixed inset-0 z-[200] flex justify-end" style={{ backgroundColor: "rgba(0,0,0,0.3)" }} onClick={onClose}>
      <div className="bg-white w-full max-w-sm h-full flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
          <p className="text-[11px] tracking-[0.25em] text-stone-800 uppercase">Wishlist ({wishlist.length})</p>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-6">
          {wishlist.length === 0 && <p className="text-[12px] text-stone-400 text-center mt-16 tracking-wide">No saved items yet</p>}
          {wishlist.map((item, i) => (
            <div key={i} className="flex gap-4">
              ...
              <div className="flex flex-col justify-between flex-1 py-0.5">
                <div>
                  ...
                  <p className="text-[10px] text-stone-400 mt-0.5">PKR {item.price.toLocaleString()}</p>
                </div>
                <div className="flex items-center justify-between">
                  <button onClick={() => onMoveToCart(item, i)} className="text-[10px] tracking-wide text-stone-600 hover:text-stone-900 uppercase underline underline-offset-2 transition-colors">Move to Bag</button>
...
function Hero() {
  return (
    <section className="relative w-full h-[92vh] overflow-hidden">
      <img src="/background.jpg" alt="Night Elegance Campaign" className="absolute inset-0 w-full h-full object-cover object-top" />
      <div className="absolute inset-0 bg-stone-900/20" />
      <div className="absolute bottom-12 left-8 md:left-14">
        <p className="text-[10px] tracking-[0.35em] text-stone-200 uppercase mb-2">New Collection 2026</p>
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-light tracking-wide text-white leading-none">Night<br />Elegance</h1>
        <a href="#collection" className="inline-block mt-8 text-[10px] tracking-[0.3em] text-stone-200 uppercase border-b border-stone-300 pb-0.5 hover:text-white hover:border-white transition-colors">Discover</a>
      </div>
      <div className="absolute top-6 right-8 md:right-14">
        <p className="text-[10px] tracking-[0.25em] text-stone-300 uppercase">SS 2026</p>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="overflow-hidden border-y border-stone-100 py-4 bg-white">
      <div className="flex gap-32 animate-marquee whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="text-[10px] tracking-[0.35em] text-stone-400 uppercase flex-shrink-0">{item}</span>
        ))}
      </div>
    </div>
  );
...
function CollectionBanner() {
  return (
    <section className="relative w-full h-[80vh] overflow-hidden">
      <img src="/bg1.jpg" alt="Soft Nights Collection" className="absolute inset-0 w-full h-full object-cover" />
      <div className="absolute inset-0 bg-stone-900/30" />
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
        <p className="text-[10px] tracking-[0.4em] text-stone-200 uppercase mb-4">The Soft Nights Edit</p>
        <h2 className="text-5xl md:text-7xl font-light tracking-widest text-white uppercase leading-none mb-10">Soft Nights</h2>
        <a href="#" className="text-[10px] tracking-[0.3em] text-stone-200 uppercase border-b border-stone-300 pb-0.5 hover:text-white transition-colors">Explore Collection</a>
      </div>
    </section>
  );
}
...
function EditorialStrip() {
  return (
    <section className="px-6 md:px-12 py-24 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
      <img src="/bg2.jpg" alt="Editorial" className="h-[500px] md:h-[680px] w-full object-cover" />
      <div className="flex flex-col gap-6 md:pr-16">
        ...
        <h2 className="text-3xl md:text-4xl font-light tracking-wide text-stone-800 leading-snug">From late-night chai<br /> to slow, peaceful mornings</h2>
        <p className="text-[13px] text-stone-400 leading-relaxed max-w-xs">Our pieces are made for the rhythm of your everyday life. Inspired by the softness of Pakistani nights, each design brings together breathable fabrics, effortless silhouettes, and a touch of quiet luxury.</p>
        <a href="#" className="inline-block mt-4 text-[10px] tracking-[0.3em] text-stone-600 uppercase border-b border-stone-400 pb-0.5 w-fit hover:text-stone-900 hover:border-stone-900 transition-colors">Our Story</a>
      </div>
    </section>
  );
}
...
export default function TwinklePage() {
  ...
  // Local UI state (sidebars/modals — these are fine as local state)
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <Hero />
      <Marquee />
      <ProductGrid onProductClick={setActiveProduct} wishlist={wishlist} onWishlistToggle={handleWishlistToggle} />
      <CollectionBanner />
      <EditorialStrip />
      <Footer />
      ...
      {activeProduct && (
        <ProductModal
          ...
          onAddToCart={(item) => { handleAddToCart(item); setActiveProduct(null); setCartOpen(true); }}
          onWishlistToggle={handleWishlistToggle}
          wishlisted={wishlist.some((w) => w.id === activeProduct.id)}
        />
      )}

      {cartOpen && <CartSidebar cart={cart} onClose={() => setCartOpen(false)} onRemove={handleRemoveFromCart} />}
