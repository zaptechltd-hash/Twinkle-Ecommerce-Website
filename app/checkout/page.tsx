...
//     // Map cart items → CreateOrderItemPayload
//     const items: CreateOrderItemPayload[] = cart.map((item: any) => {
//       const variant =
//         item.variants?.find((v: any) => v.color === item.selectedColor) ??
//         item.variants?.[0];

//       // Some backends store sku per size, others per variant — handle both
...
//                   <Link href="/" className="underline underline-offset-2 hover:text-stone-700 transition-colors">
//                     Sign in
//                   </Link>
//                 </p>
//               </>
...
//                   <p className="text-[10px] text-stone-400 mt-0.5">Estimated delivery in 2–5 working days</p>
//                 </div>
//               </div>
...
//                           {opt.icons.map((ic) => (
//                             <span key={ic} className="text-[8px] tracking-[0.1em] border border-stone-200 px-1.5 py-0.5 text-stone-500 font-medium">
//                               {ic}
//                             </span>
//                           ))}
//                         </div>
...
//                   className="flex-1 border border-stone-200 border-r-0 rounded-l-sm px-3 py-2.5 text-[12px] text-stone-700 outline-none focus:border-stone-500 focus:ring-2 focus:ring-stone-500/10 transition-colors placeholder:text-stone-300"
//                 />
//                 <button className="px-4 py-2.5 bg-stone-900 text-white text-[10px] tracking-[0.2em] uppercase hover:bg-stone-700 transition-colors flex-shrink-0 rounded-r-sm">
//                   Apply
//                 </button>
//               </div>
//             </div>
...
//                   You're saving PKR 250 on shipping
//                 </p>
//               </div>
//             )}
//           </div>
...
export default function CheckoutPage() {
  ...
  const set = (key: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    setForm((f) => ({ ...f, [key]: e.target.value }));
    if (errors[key]) setErrors((prev) => { const n = { ...prev }; delete n[key]; return n; });
    // Reset coupon state if user clears the code field
  ...
  const handleSubmit = async () => {
    ...
    const items: CreateOrderItemPayload[] = cart.map((item: any) => {
      const variant =
        item.variants?.find((v: any) => v.color === item.selectedColor) ??
        item.variants?.[0];
      const sizeEntry = variant?.sizes?.find((s: any) => s.size === item.selectedSize);
      return {
        productId:     item.id,
        productName:   item.name,
        variantId:     variant?.id ?? "",
    ...
    const payload: CreateOrderPayload = {
      ...
      phone:         form.phone,
      paymentMethod: form.paymentMethod,
      ...(appliedCode     && { discountCode: appliedCode }),  // only send if actually validated
      items,
    };
        ...
        <aside className="lg:sticky lg:top-24 flex flex-col gap-0">
          <div className="bg-white border border-stone-100">
            ...
            <div className="px-6 py-5 flex flex-col gap-3">
              ...
              {/* Discount row — only shown when applied */}
              {discountAmount > 0 && (
                <div className="flex justify-between items-center">
