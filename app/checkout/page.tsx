    ...
    <header className="border-b border-stone-100 bg-white sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-6 md:px-12 py-4 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3">
          ...
          />
          <span className="text-[18px] font-light tracking-[0.4em] text-stone-800 uppercase hidden" style={{ display: "none" }}>
            Twinkle
          </span>
        </Link>
...
function SectionTitle({ number, title, subtitle }: { number: string; title: string; subtitle?: string }) {
  return (
    <div className="flex items-start gap-3 mb-5">
      ...
      <div>
        <p className="text-[12px] tracking-[0.2em] text-stone-800 uppercase font-medium">{title}</p>
...
function InputField({
  label, type = "text", placeholder, value, onChange, required, error, className = "",
}: {
...
export default function CheckoutPage() {
  ...
  const set = (key: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (key === "discountCode" && appliedCode) {
      setAppliedCode(""); setDiscountAmount(0); setCouponSuccess(""); setCouponError("");
    }
  };
  ...
  const handleApplyCoupon = async () => {
    ...
    if (!res || !res.valid) {
      ...
      setDiscountAmount(0); setAppliedCode(""); return;
    }
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-10 grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10 items-start">
        ...
        <div className="flex flex-col gap-8">
            ...
            {user ? (
              <div className="flex items-center justify-between py-3 px-4 bg-stone-50 border border-stone-100 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[12px] text-stone-600">{user.email}</span>
                </div>
                <Link href="/" className="text-[10px] tracking-[0.15em] text-stone-400 uppercase underline underline-offset-2 hover:text-stone-700">
                  Change
                </Link>
              </div>
            ) : (
              <>
                <InputField
                  ...
                  required error={errors.email} className="mb-4"
                />
                <p className="text-[10px] text-stone-400 mb-1">
                  Already have an account?{" "}
                  <Link href="/" className="underline underline-offset-2 hover:text-stone-700 transition-colors">Sign in</Link>
                </p>
              </>
            )}
            <label className="flex items-center gap-2.5 cursor-pointer mt-3">
              <input
                type="checkbox" checked={form.emailOffers}
                onChange={(e) => setForm((f) => ({ ...f, emailOffers: e.target.checked }))}
                className="w-3.5 h-3.5 accent-stone-800"
              />
              <span className="text-[11px] text-stone-500 tracking-wide">Email me with news and offers</span>
            </label>
          </section>
          ...
          <section className="bg-white border border-stone-100 p-6 md:p-8">
            ...
            <div className="mb-4">
              <label className="block text-[10px] tracking-[0.2em] uppercase text-stone-400 mb-1.5">Country</label>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <InputField label="First Name" placeholder="Sara"  value={form.firstName} onChange={set("firstName")} required error={errors.firstName} />
              <InputField label="Last Name"  placeholder="Noor"  value={form.lastName}  onChange={set("lastName")}  required error={errors.lastName}  />
            </div>
            <InputField label="Address"              placeholder="Street address" value={form.address}   onChange={set("address")}   required error={errors.address}  className="mb-4" />
            <InputField label="Apartment, suite, etc." placeholder="Optional"    value={form.apartment} onChange={set("apartment")}                                   className="mb-4" />
            <InputField label="City"                 placeholder="e.g. Karachi"  value={form.city}      onChange={set("city")}      required error={errors.city}     className="mb-4" />
            <InputField label="Postal Code"          placeholder="74400"         value={form.postalCode} onChange={set("postalCode")}                                 className="mb-4" />
            <div className="mb-5">
              <label className="block text-[10px] tracking-[0.2em] uppercase text-stone-400 mb-1.5">
              ...
              <div className="flex">
                ...
                <input
                  type="tel" placeholder="0332 3520504" value={form.phone} onChange={set("phone")}
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input
                type="checkbox" checked={form.saveInfo}
                onChange={(e) => setForm((f) => ({ ...f, saveInfo: e.target.checked }))}
                className="w-3.5 h-3.5 accent-stone-800"
              />
              <span className="text-[11px] text-stone-500 tracking-wide">Save this information for next time</span>
            </label>
          </section>
          ...
          <section className="bg-white border border-stone-100 p-6 md:p-8">
            ...
            <div className="border border-stone-200 rounded-sm px-4 py-4 flex items-center justify-between bg-stone-50">
              <div className="flex items-center gap-3">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a8a29e" strokeWidth="1.6">
                ...
                <div>
                  <p className="text-[12px] text-stone-800 tracking-wide">Standard Delivery</p>
                  <p className="text-[10px] text-stone-400 mt-0.5">{deliveryLabel}</p>
                </div>
              </div>
              {shipping === 0 ? (
                <p className="text-[11px] text-green-600 font-medium tracking-wide">Free</p>
              ) : (
                <p className="text-[11px] text-stone-700 font-medium tracking-wide">PKR {shipping.toLocaleString()}</p>
              )}
            </div>
          </section>
          ...
          {/* 4. Payment */}
          <section className="bg-white border border-stone-100 p-6 md:p-8">
            <SectionTitle number="4" title="Payment" subtitle="All transactions are secure and encrypted." />
              ...
              {paymentOptions.map((opt, i) => (
                <label
                  ...
                  className={`flex items-start gap-3 px-4 py-4 cursor-pointer transition-colors ${
                    form.paymentMethod === opt.id ? "bg-stone-50" : "bg-white hover:bg-stone-50/50"
                  } ${i > 0 ? "border-t border-stone-100" : ""}`}
                >
                  <input
                    type="radio" name="payment" value={opt.id}
                    checked={form.paymentMethod === opt.id}
                    onChange={() => setForm((f) => ({ ...f, paymentMethod: opt.id as "card" | "cod" }))}
                    className="mt-0.5 accent-stone-800"
                  />
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <p className="text-[12px] text-stone-800 tracking-wide">{opt.label}</p>
                      {opt.icons.length > 0 && (
                        <div className="flex gap-1">
                          {opt.icons.map((ic) => (
                            <span key={ic} className="text-[8px] tracking-[0.1em] border border-stone-200 px-1.5 py-0.5 text-stone-500 font-medium">{ic}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    {form.paymentMethod === opt.id && (
                      <p className="text-[10px] text-stone-400 mt-1.5 leading-relaxed">{opt.description}</p>
                    )}
                  </div>
                </label>
              ))}
          ...
          <div className="flex flex-col gap-2 px-1">
            {[
              ...
              <p key={note} className="text-[10px] text-stone-400 tracking-wide leading-relaxed flex gap-2">
            ...
            <p className="text-[10px] text-stone-400 mt-1">
              For inquiries:{" "}
              <a href="mailto:support@twinkleofficial.com" className="underline underline-offset-2 hover:text-stone-700">
        ...
        <aside className="lg:sticky lg:top-24 flex flex-col gap-0">
          <div className="bg-white border border-stone-100">
            <div className="px-6 py-4 border-b border-stone-100">
              <p className="text-[11px] tracking-[0.25em] text-stone-800 uppercase">Order Summary</p>
            </div>

            {/* Items */}
            <div className="px-6 py-4 flex flex-col gap-5 border-b border-stone-100">
              {cart.length === 0 && (
                <p className="text-[12px] text-stone-400 text-center py-4">Your bag is empty</p>
              )}
                ...
                return (
                  <div key={i} className="flex gap-3">
                    <div className="relative flex-shrink-0">
                      <img src={imageSrc} alt={item.name} className="w-16 h-20 object-cover" />
                    ...
                    <div className="flex flex-col justify-center flex-1 gap-0.5">
                      <p className="text-[11px] tracking-[0.12em] text-stone-800 uppercase leading-snug">{item.name}</p>
                      <p className="text-[10px] text-stone-400">{item.selectedSize} · Qty: {item.qty}</p>
                    </div>
                    <div className="flex flex-col items-end justify-center gap-0.5 flex-shrink-0">
                      <p className="text-[12px] text-stone-700 tracking-wide">PKR {(effectivePrice * item.qty).toLocaleString()}</p>
                      {item.discountPrice != null && item.discountPrice < item.price && (
                        <p className="text-[10px] text-stone-300 line-through">PKR {(item.price * item.qty).toLocaleString()}</p>
                      )}
                    </div>
                  </div>
                );
            ...
            {/* Discount Code */}
            <div className="px-6 py-4 border-b border-stone-100">
              <p className="text-[10px] tracking-[0.2em] uppercase text-stone-400 mb-2">Discount Code</p>
              {appliedCode ? (
                <div className="flex items-center justify-between py-2.5 px-3 bg-green-50 border border-green-200 rounded-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-green-700 font-medium tracking-wider">{appliedCode}</span>
                    <span className="text-[10px] text-green-600">— PKR {discountAmount.toLocaleString()} off</span>
                  </div>
                  <button onClick={handleRemoveCoupon} className="text-[10px] text-stone-400 hover:text-red-400 transition-colors underline underline-offset-2">
                    Remove
                  </button>
                </div>
              ) : (
                <>
                  <div className="flex gap-0">
                    <input
                      type="text" placeholder="Enter code" value={form.discountCode}
            ...
            <div className="px-6 py-5 flex flex-col gap-3">
              <div className="flex justify-between">
                <p className="text-[11px] tracking-[0.15em] text-stone-500 uppercase">Subtotal</p>
                <p className="text-[12px] text-stone-700">PKR {subtotal.toLocaleString()}</p>
              </div>
              {discountAmount > 0 && (
                <div className="flex justify-between items-center">
                  <p className="text-[11px] tracking-[0.15em] text-green-600 uppercase">Discount ({appliedCode})</p>
                  <p className="text-[12px] text-green-600 font-medium">− PKR {discountAmount.toLocaleString()}</p>
                </div>
              )}
              <div className="flex justify-between items-center">
                <p className="text-[11px] tracking-[0.15em] text-stone-500 uppercase">Shipping</p>
                {shipping === 0 ? (
                  <p className="text-[11px] text-green-600 font-medium">Free</p>
                ) : (
                  <p className="text-[12px] text-stone-700">PKR {shipping.toLocaleString()}</p>
                )}
              </div>
              <div className="border-t border-stone-100 pt-3 mt-1">
                <div className="flex justify-between items-baseline">
                  <div>
                    <p className="text-[12px] tracking-[0.2em] text-stone-800 uppercase font-medium">Total</p>
                    <p className="text-[10px] text-stone-400 mt-0.5">Incl. PKR {tax.toLocaleString()} in taxes</p>
                  </div>
                  <p className="text-[16px] text-stone-900 tracking-wide">PKR {total.toLocaleString()}</p>
                </div>
              </div>
            </div>
