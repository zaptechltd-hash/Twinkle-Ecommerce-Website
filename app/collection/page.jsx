        ...
        <div className="flex flex-col gap-1">
          {PRICE_RANGES.map((range, i) => {
            ...
            return (
              <button
                key={i}
                onClick={() => onChange({ ...filters, priceRange: active ? null : i })}
              ...
              >
                <span
                  className={`w-3.5 h-3.5 border flex-shrink-0 flex items-center justify-center transition-all ${
                    active ? "border-stone-800 bg-stone-800" : "border-stone-200"
                  }`}
                >
                  {active && (
                    <svg width="7" height="7" viewBox="0 0 10 10">
                      <polyline points="1,5 4,8 9,2" stroke="white" strokeWidth="1.5" fill="none" />
                    </svg>
                  )}
                </span>
                {range.label}
              ...
              >
                <span
                  className={`w-5 h-5 rounded-full border-2 transition-all flex-shrink-0 ${
                    active ? "border-stone-700 scale-110" : "border-transparent"
                  }`}
                  style={{
                    backgroundColor: col.hex,
                    boxShadow: active ? "0 0 0 1px #79716b" : "0 0 0 1px #e7e5e4",
                  }}
                />
                <span
                  className={`text-[11px] tracking-wide transition-colors ${
                    active ? "text-stone-900" : "text-stone-400 group-hover:text-stone-700"
                  }`}
                >
                  {col.name}
                </span>
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
          <FilterPanel filters={filters} onChange={onChange} onClear={onClear} totalResults={totalResults} />
        </div>
...
function ActiveFilters({ filters, onChange, onClear }) {
  ...
  filters.fabrics.forEach((f) =>
    pills.push({ label: f, clear: () => onChange({ ...filters, fabrics: filters.fabrics.filter((x) => x !== f) }) })
  );
  filters.colors.forEach((c) =>
    pills.push({ label: c, clear: () => onChange({ ...filters, colors: filters.colors.filter((x) => x !== c) }) })
  );
  filters.sizes.forEach((s) =>
    pills.push({ label: s, clear: () => onChange({ ...filters, sizes: filters.sizes.filter((x) => x !== s) }) })
  );
  if (filters.priceRange !== null)
    pills.push({ label: PRICE_RANGES[filters.priceRange].label, clear: () => onChange({ ...filters, priceRange: null }) });
...
<Navbar/>
      ...
      <div className="px-6 md:px-12 pt-10 pb-0">
        {/* <p className="text-[9px] tracking-[0.25em] text-stone-400 uppercase mb-6">
          Home / <span className="text-stone-600">Nightwear</span>
        </p> */}
      ...
      <div className="px-6 md:px-12 py-4 flex items-center justify-between border-b border-stone-100 sticky top-0 bg-white z-20">
        ...
        {/* Desktop: spacer to push sort right */}
