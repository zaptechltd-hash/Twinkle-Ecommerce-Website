...
import type { Order, OrderStatus, OrderQueryParams } from "../../../services/order/types";
...
function PaymentBadge({ status }: { status: string }) {
  return (
    <span className={`text-[10px] font-medium px-2 py-0.5 rounded ${PAYMENT_STYLES[status] ?? ""}`}>
      {status}
    </span>
  );
}
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            {[
              ...
              ["Email",       <span className="text-[#185fa5] break-all">{order.email}</span>],
              ["Phone",       order.phone],
              ["Date",        formatDate(order.createdAt)],
              ["Payment",     <PaymentBadge status={order.paymentStatus} />],
              ["Method",      <span className="uppercase text-[11px] font-medium">{order.paymentMethod}</span>],
              ["Order Total", <span className="font-medium">PKR {order.totalAmount.toLocaleString()}</span>],
          ...
          <div className="mb-5 bg-[#f9f8f6] border border-[#e8e5df] rounded-xl px-4 py-3">
            <p className="text-[10px] tracking-[0.1em] uppercase text-[#b4b2a9] mb-1.5">Delivery Address</p>
            <p className="text-[12px] text-[#1a1916] leading-relaxed">
              {order.address}{order.apartment ? `, ${order.apartment}` : ""}<br />
              {order.city}{order.postalCode ? ` ${order.postalCode}` : ""} · {order.country}
            </p>
          </div>
          ...
          <div className="mb-5">
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#e8e5df]">
              <p className="text-[11px] font-medium text-[#5f5e5a]">Order Total</p>
              <p className="text-[13px] font-medium text-[#1a1916]">PKR {order.totalAmount.toLocaleString()}</p>
            </div>
          </div>
          ...
          {order.discountCode && (
            <div className="mb-5">
              <p className="text-[10px] tracking-[0.1em] uppercase text-[#b4b2a9] mb-1">Discount Code</p>
...
function BulkBar({ count, onAction, onClear, disabled }: {
  count: number;
...
export default function OrderManagement() {
  ...
  const handleBulkAction = async (newStatus: string) => {
    ...
    setUpdating(true);
    try {
      await Promise.all(ids.map((id) => updateOrder(id, { status: newStatus as OrderStatus })));
      // Refresh current page
      await fetchOrders({
        page, limit: PER_PAGE, sort,
        ...(activeStatus !== "All" && { status: activeStatus as OrderStatus }),
        ...(payFilter !== "All" && { paymentStatus: payFilter as any }),
        ...(search && { search }),
      });
  ...
  const toggleAll  = (chk: boolean) => setSelected((p) => { const n = new Set(p); orders.forEach((o) => chk ? n.add(o.id) : n.delete(o.id)); return n; });
      ...
      <div className="bg-[#f5f2ed] min-h-screen p-6 md:p-8 text-[#1a1916]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {fetching ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="bg-white border border-[#e8e5df] rounded-xl p-4">
        ...
        <div className="bg-white border border-[#e8e5df] rounded-xl p-5">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            ...
            <select
              value={sort}
              onChange={(e) => { setSort(e.target.value as "newest" | "oldest"); resetPage(); }}
              className="text-[12px] px-3 py-2 border border-[#e8e5df] rounded-xl bg-white text-[#5f5e5a] outline-none"
            >
                    ...
                    >
                      ...
                      <td className="py-3 pr-4">
                        <div className="flex items-center gap-2">
                          <Initials name={name} />
                          <div>
                            <p className="text-[12px] font-medium text-[#1a1916]">{name}</p>
                            <p className="text-[10px] text-[#b4b2a9]">{order.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-3 pr-4">
                      ...
                      <td className="py-3 pr-4 text-center">
                        <span className="text-[12px] text-[#5f5e5a]">{totalItems}</span>
                      </td>
                      <td className="py-3 pr-4">
          ...
          <div className="flex items-center justify-between mt-5 flex-wrap gap-3">
            <p className="text-[11px] text-[#b4b2a9]">
              Showing {meta.total === 0 ? 0 : (page - 1) * PER_PAGE + 1}–{Math.min(page * PER_PAGE, meta.total)} of {meta.total}
            </p>
            <div className="flex items-center gap-1">
              <button
              ...
              {Array.from({ length: meta.totalPages }, (_, i) => i + 1).map((n) => (
                <button
                  ...
                  onClick={() => setPage(n)}
                  className={`text-[11px] font-medium w-8 h-8 rounded-lg transition-all ${
                    n === page ? "bg-[#1a1916] text-[#f5f2ed]" : "text-[#5f5e5a] hover:bg-[#f1efe8]"
                  }`}
                >
                  {n}
                </button>
